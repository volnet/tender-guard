...
test('HTML operation report includes PreSalesX branding and saved actions',async()=>{const dir=await fsp.mkdtemp(path.join(os.tmpdir(),'pg-html-')),file=path.join(dir,'report.html');await exportHtml(project(),file);const html=await fsp.readFile(file,'utf8');assert.match(html,/PreSalesX/);assert.match(html,/操作记录/);assert.match(html,/a-已修改\.pdf/);await fsp.rm(dir,{recursive:true,force:true});});
