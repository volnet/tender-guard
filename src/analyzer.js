...
function coreProps(xml) {
  ...
  return { author:text(c['dc:creator']||a.Company), lastEditor:text(c['cp:lastModifiedBy']||a.Manager), title:text(c['dc:title']), subject:text(c['dc:subject']), keywords:text(c['cp:keywords']), description:text(c['dc:description']), created:text((c['dcterms:created']||{})['#text']||c['dcterms:created']), modified:text((c['dcterms:modified']||{})['#text']||c['dcterms:modified']), lastPrinted:text(c['cp:lastPrinted']), company:text(a.Company), manager:text(a.Manager), application:text(a.Application), appVersion:text(a.AppVersion), template:text(a.Template), totalEditingTime:text(c['cp:totalTime']), revision:text(c['cp:revision'])};
}
async function officeExtract(buffer, kind) {
  ...
  try {
    ...
    if(kind==='excel') {
      ...
      const sheets=all.filter(([n])=>/^xl\/worksheets\/sheet\d+\.xml$/.test(n)); props.formulas=sheets.flatMap(([n,b])=>Array.from(b.toString().matchAll(/<f[^>]*>([\s\S]*?)<\/f>/g),x=>({source:n,value:x[1]}))).slice(0,5000); props.formulaHash=sha(Buffer.from(props.formulas.map(x=>normalize(x.value)).join('\n')));
    }
...
function propertyGroups(f) {
  ...
  return [
    {id:'common',title:'常见文件属性',open:true,items:useful({文件名:f.name,'ZIP 内路径':f.zipPath,文件类型:f.kind,MIME:f.mime,扩展名:f.extension,文件大小:f.size,'SHA-256':f.sha256,作者:a.author,'最后一次保存作者':a.lastEditor,创建时间:a.created,编辑时间:a.modified,最后打印时间:a.lastPrinted,应用程序:a.application,版本号:a.appVersion,公司:a.company,管理者:a.manager})},
    {id:'document',title:f.kind==='pdf'?'PDF 文档属性':f.kind==='excel'?'Excel 专有属性':f.kind==='word'?'Word 专有属性':f.kind==='powerpoint'?'PowerPoint 专有属性':'文档专有属性',open:true,items:useful({标题:a.title,主题:a.subject,关键词:a.keywords,说明:a.description,模板:a.template,修订版本:a.revision,总编辑时间:a.totalEditingTime,'PDF 版本':a.pdfVersion,页数:a.pageCount,文档ID:a.documentIds,工作表:a.sheets,公式数量:a.formulas?.length,公式结构哈希:a.formulaHash})},
...
function correlate(submissions, enabled={}) {
  ...
  for(let i=0;i<fsx.length;i++)for(let j=i+1;j<fsx.length;j++){const a=fsx[i],b=fsx[j];if(a.supplier===b.supplier)continue;
    ...
    const pairs=[['author','相同作者',45],['lastEditor','相同最后编辑人',75],['company','相同公司属性',55],['template','相同模板路径',75],['internalStructureHash','相同内部结构',45],['formulaHash','相同公式结构',70]];
    for(const [k,t,w] of pairs){if(!allow(k==='formulaHash'?'formula':'metadata'))continue;const x=normalize(a.attributes[k]),y=normalize(b.attributes[k]);if(x&&x===y&&!['microsoft office user','administrator','admin','wps'].includes(x))add(t,a.attributes[k],a,b,w,`不同供应商文件的${t.replace('相同','')}一致`,a.zipPath,b.zipPath);}
